...
- 数据同步 / 云备份
- 多语言
- **引入 Jetpack Compose / SwiftUI 等原生 UI 框架**（用户要求搁置到最终阶段评估）

---

## 2. 平台视觉适配策略

Phase 3 在保留 Flash 自身设计语言的前提下，通过 CSS/Tailwind 设计 token 与平台检测，让 Android 端呈现 Material Design 3 风格、iOS 端呈现 Apple / Liquid Glass 风格，Web 预览根据 User Agent 自动切换。

### 2.1 平台检测

- 新增 `src/lib/platform.ts`，基于 `Capacitor.getPlatform()` 与 `navigator.userAgent` 返回 `'android' | 'ios' | 'web'`。
- Web 端优先检测 UA：包含 `Android` 则视为 Android，包含 `iPhone|iPad|iPod|Macintosh` 则视为 iOS，否则默认 iOS。
- 在 `App.tsx` 初始化时将平台类名写入 `<html>` 或 `<body>`，如 `platform-android` / `platform-ios`，供全局 CSS 选择器使用。

### 2.2 设计 Token 映射

| 视觉属性 | Android (Material 3) | iOS (Liquid Glass) |
|----------|----------------------|--------------------|
| 主圆角 | 16px（卡片/按钮），28px（底部 Sheet） | 12px（卡片），22px（底部 Sheet） |
| 阴影/ elevation | 多层柔和投影，强调层级 | 极淡或无边框阴影，依赖 backdrop blur |
| 背景材质 | 半透明深色 + 轻微 elevation | 高斯模糊玻璃（`backdrop-blur-xl`）+ 细白边框 |
| 强调色 | Material Indigo / Violet（`#6750A4`） | Apple 系统蓝（`#0A84FF`） |
| 按钮形状 | 圆角胶囊/矩形，涟漪反馈 | 圆角矩形，微缩放按压 |
| 动画曲线 | `cubic-bezier(0.2, 0, 0, 1)` 强调 | `cubic-bezier(0.32, 0.72, 0, 1)` 弹簧感 |
| 底部导航 | 悬浮胶囊 / 圆角 bar | 半透明毛玻璃 tab bar |
| 输入框 | 底部下划线或填充框 | 无边框 + 分隔线 |

### 2.3 实现方式

- 在 `src/index.css` 定义两套 CSS 变量：`.platform-android` 与 `.platform-ios`。
- 组件内用 Tailwind 的任意值引用变量，例如 `rounded-[var(--radius-card)]`、`bg-[var(--color-primary)]`。
...
- 在 Android release APK 与 iOS Simulator 上分别验证两套视觉是否正确生效。
...
| 用户未来要求引入 Compose/SwiftUI | 低 | Phase 3 仅做 CSS 层适配，原生框架重构列为 Phase 5 评估项 |
...
- [ ] Web 预览根据 User Agent 切换 Android/iOS 视觉 token；真机打包后各自呈现对应平台风格。
